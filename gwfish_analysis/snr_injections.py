...
out_file_name = totaldir+'snr_'+opts.det+'_'+str(opts.num)+'_'+str(opts.inj)+'.txt'
print(out_file_name)
if os.path.exists(out_file_name):
  # Here, we assume the calculations for other detectors are also complete
  lho = np.loadtxt(out_file_name)
  llo = np.loadtxt(out_file_name.replace('LHO','LLO'))
  vir = np.loadtxt(out_file_name.replace('LHO','VIR'))
  voy = np.loadtxt(out_file_name.replace('LHO','VOY'))
  snr_hlv = np.sqrt(lho**2 + llo**2 + vir**2)
  snr_voyv = np.sqrt(voy**2 + vir**2)
  ...
  np.savetxt(out_file_name, snr_odd_sph)
