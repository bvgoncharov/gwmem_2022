...
def alternative_hdf_load(pardir):
    with h5py.File(pardir, 'r') as f:
        # Accessing the root group and then the datasets
        root_group = f['root']
        axis0 = root_group['axis0'][:]  # Index/row labels
        axis1 = root_group['axis1'][:]  # Column labels
        block0_values = root_group['block0_values'][:]  # Data

        # Decoding byte strings to Unicode strings
...
try:
    pd.read_hdf('/fred/oz031/mem/gwmem_2022_container/image_content/pops_gwmem_2022/pop_max_o3_bbh_only_1yr_20230606_sorted_snrjj.hdf5')
    read_hdf = pd.read_hdf
except ImportError:
    print('ImportError for pd.read_hdf(), attempting to use an alternative')
    read_hdf = alternative_hdf_load
