...
for ii, kk in zip([3], ['red','green','blue','black','purple']):
  ...
  if kk=='black':
    ...
    labelp, labelx = None, None
  axs[0,0].plot(td_strain[:,0], np.real(td_strain[:,1] -1j*td_strain[:,2]), color='#0E81CE', lw=0.8) # total TD
  axs[0,0].plot(td_strain[:,0], np.imag(td_strain[:,1] -1j*td_strain[:,2]), color='#CE0E81', lw=0.8, ls='--') # total TD
  axs[0,1].plot(td_strain[:,0], np.real(td_strain[:,3] -1j*td_strain[:,4]), color='#0E81CE', lw=0.8) # disp TD
  axs[0,1].plot(td_strain[:,0], np.imag(td_strain[:,3] -1j*td_strain[:,4]), color='#CE0E81', lw=0.8, ls='--') # disp TD
  axs[0,2].plot(td_strain[:,0], np.real(td_strain[:,5] -1j*td_strain[:,6]), color='#0E81CE', lw=0.8, label=labelp) # spin TD
  axs[0,2].plot(td_strain[:,0], np.imag(td_strain[:,5] -1j*td_strain[:,6]), color='#CE0E81', lw=0.8, ls='--', label=labelx) # spin TD

  axs[1,0].loglog(fd_strain[:,0], fd_strain[:,1], color='#81CE0E', lw=0.8)
  axs[1,1].loglog(fd_strain[:,0], fd_strain[:,2], color='#81CE0E', lw=0.8)
  #disp_mem_approx =4e-24*fd_strain[0,0]/fd_strain[:,0]
  #axs[1,1].fill_between(fd_strain[:,0],y1=disp_mem_approx,y2=2e-25,color='#81CE0E',alpha=0.3,lw=0.)
  axs[1,2].loglog(fd_strain[:,0], fd_strain[:,3], color='#81CE0E', lw=0.8)

#disp_mem_approx = 7e-24*fd_strain[0,0]/fd_strain[:,0]
#axs[1,1].loglog(fd_strain[:,0],disp_mem_approx,color='yellow',ls=':')

axs[0,0].set_xlim([-0.1,np.max(td_strain[:,0])])
axs[0,1].set_xlim([-0.1,np.max(td_strain[:,0])])
axs[0,2].set_xlim([-0.1,np.max(td_strain[:,0])])
...
axs[1,0].set_ylabel('Strain, $\\tilde{h}(f)$ [s]', fontdict=font)
